...
import psycopg2
conn = psycopg2.connect(database="allotment", user = "postgres", password = "Logapriya@213", host = "127.0.0.1", port = "5432")
# conn = psycopg2.connect(database="servicebot", user = "cloobot", password = "cloobot", host = "127.0.0.1", port = "5432")

import ast
cur = conn.cursor()


def get_page_data(from_no,to_no,page_no,table_name,search_text,required,user_org_id):
    if(search_text == ""):
        cur.execute("select * from "+table_name+" where org_id = %s ORDER BY timestamp DESC;",(user_org_id,))
...
def get_page_no(table_name,search_text,user_org_id):
    if(search_text == ""):
        cur.execute("select * from "+table_name+" where org_id = %s ;",(user_org_id,))
        table_data = cur.fetchall()
        # print(table_data)
        try:
...
def search(data,search_text,all_cols_list,datatypes_list,search_cols_list):
    try:
        ...
        pass
    print("\n\n for search dropped \n\n")
    # # cur.execute("drop trigger tcvrealtime;")
    # print("\n\n trigger dropped \n\n")
    # # cur.execute("drop index fts_idex;")
    # print("\n\n index dropped \n\n")
    with_datatype = []
    ...
    for each_row in data:
        #####################################################################################['i_id','project_name','project_details','project_docs','project_type','project_tl','project_CE']
        ...
        cur.execute("INSERT INTO for_search ("+all_col+") VALUES (%s,%s,%s,%s,%s,%s,%s)",(each_row['i_id'],each_row['project_name'],each_row['project_details'],each_row['project_docs'],each_row['project_type'],each_row['project_tl'],each_row['project_CE']))
    conn.commit()
